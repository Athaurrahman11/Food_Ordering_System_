    ...
    <div class="max-w-3xl mx-auto">
        <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
            <div class="p-8 border-b border-gray-200 dark:border-gray-800">
                <h2 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Edit Restaurant</h2>
            </div>
            
            <form action="{{ url('update_restaurant/'.$restaurant->id) }}" class="p-8 space-y-8" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex flex-col items-center gap-4 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 px-6 py-12 transition-colors hover:border-primary/50">
                        <div class="flex flex-col items-center text-center gap-2">
                            ...
                            <p class="text-gray-500 dark:text-gray-400 text-sm max-w-xs">Current: {{ $restaurant->image }}</p>
                        </div>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div class="space-y-2">
                        ...
                        <input class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-transparent text-sm" value="{{ $restaurant->name }}" type="text" name="name" required/>
                    </div>

                    <div class="space-y-2">
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Description</label>
                        <textarea class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-transparent text-sm" name="description" rows="3">{{ $restaurant->description }}</textarea>
                    </div>

                    <div class="space-y-2">
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Delivery Time</label>
                        <input class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-transparent text-sm" value="{{ $restaurant->delivery_time }}" type="text" name="delivery_time" required/>
                    </div>
                </div>
