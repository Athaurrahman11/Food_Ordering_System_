...
<h2 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Edit Food Item</h2>
</div>
...
<form action="{{ url('update_food/'.$food->id) }}" class="p-8 space-y-8" method="POST" enctype="multipart/form-data">
@csrf
...
<p class="text-gray-900 dark:text-white text-lg font-bold">Upload New Image (Optional)</p>
<p class="text-gray-500 dark:text-gray-400 text-sm max-w-xs">Current Image: {{ $food->image }}</p>
</div>
...
<input class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-transparent text-sm" value="{{ $food->name }}" type="text" name="name" required/>
</div>
...
@foreach ($items as $item)
    <option value="{{ $item->id }}" {{ $food->menu_id == $item->id || $food->category == $item->category ? 'selected' : '' }}>{{ $item->category }}</option>
@endforeach
...
<input class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg pl-8 pr-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-transparent text-sm" value="{{ $food->price }}" step="0.01" type="number" name="price" required/>
</div>
</div>
<!-- Quantity / Stock -->
<div class="md:col-span-2 space-y-2">
<label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Quantity / Stock</label>
<input class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-transparent text-sm" value="{{ $food->stock }}" type="number" name="stock" required/>
</div>
</div>
<!-- Action Buttons -->
<div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-100 dark:border-gray-800">
<a href="{{ url('food') }}" class="px-6 py-2.5 rounded-lg text-sm font-bold text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" type="button">
                                    Cancel
                                </a>
